...
def cmd_analyze(args):
    ...
    risk_color = Colors.FAIL if data['riskLevel'] == 'CRITICAL' else Colors.WARNING
    print(f"Risk Score:         {risk_color}{data['riskScore']}/100 ({data['riskLevel']}){Colors.ENDC}")
    print(f"Active Sessions:    {data['activeSessionsCount']}")
    print(f"OAuth Tokens:       {data['activeTokensCount']}")
    print(f"LDAP Groups:        {data['groupsCount']} ({data['sensitiveGroupsCount']} privileged)")
    print(f"Applications:       {data['applicationsAffectedCount']} ({data['privilegedApplicationsCount']} critical/high)")
    print(f"SAML Applications:  {data['samlApplicationsCount']}")
    print(f"Access Paths:       {Colors.BOLD}{data['totalAccessPathsCount']}{Colors.ENDC} ({data['privilegedAccessPathsCount']} privileged paths)")
    
    print(f"\n{Colors.BOLD}Affected Applications:{Colors.ENDC}")
    for app in data.get('affectedApplications', []):
        sens_color = Colors.FAIL if app['sensitivityLevel'] == 'CRITICAL' else (Colors.WARNING if app['sensitivityLevel'] == 'HIGH' else Colors.BLUE)
        print(f"  • {app['applicationName']:<22} [{app['type']:<5}] Sensitivity: {sens_color}{app['sensitivityLevel']:<8}{Colors.ENDC} ({app.get('accessPathCount', 1)} paths)")
...
def cmd_simulate(args):
    ...
    print(f"Target Identity:    {data['username']}")
    print(f"Current Status:     {data['currentStatus']} ➔ Proposed: {Colors.FAIL}{data['proposedStatus']}{Colors.ENDC}")
    print(f"Disruption Score:   {Colors.FAIL if data['disruptionScore'] >= 60 else Colors.WARNING}{data['disruptionScore']}/100 ({data['disruptionLevel']}){Colors.ENDC}")
    print(f"2-Person Approval:  {Colors.BOLD}{'REQUIRED (High-Impact Identity)' if data['requiresApproval'] else 'NOT REQUIRED'}{Colors.ENDC}")
    
    print(f"\n{Colors.BOLD}Proposed Containment Actions:{Colors.ENDC}")
    for action in data.get('actionSummary', []):
        print(f"  ➔ {action}")
...
def cmd_contain(args):
    ...
    if res.get('requiresApproval'):
        print(f"{Colors.WARNING}⚠️  CONTAINMENT PENDING APPROVAL{Colors.ENDC}")
        ...
        print(f"  accesszero approve --operation {res['operationId']} --admin <secondary_admin>")
    else:
        print(f"{Colors.GREEN}✓ Containment Workflow Completed!{Colors.ENDC}")
        print(f"Operation ID:       #{res['operationId']}")
        print(f"Final Status:       {Colors.BOLD}{res['status']}{Colors.ENDC}")
        print(f"Access Paths:       {res['accessPathsRevoked']} / {res['accessPathsFound']} Revoked")
        print(f"Manual Actions:     {res['requiresManualAction']}")
...
def cmd_approve(args):
    ...
    print(f"{Colors.GREEN}✓ Operation #{op_id} APPROVED & EXECUTED!{Colors.ENDC}")
    print(f"Target User:        {res['username']}")
    print(f"Final Status:       {Colors.BOLD}{res['status']}{Colors.ENDC}")
    print(f"Access Paths:       {res['accessPathsRevoked']} / {res['accessPathsFound']} Revoked")
    for action in res.get('actionsExecuted', []):
        print(f"  ✓ {action}")
...
def cmd_verify(args):
    ...
    status_color = Colors.GREEN if data['overallStatus'] == 'CONTAINED' else Colors.WARNING
    print(f"Target:             {data['username']}")
    print(f"Overall Status:     {status_color}{Colors.BOLD}{data['overallStatus']}{Colors.ENDC}")
    print(f"Access Paths:       {data['accessPathsRevoked']} / {data['accessPathsFound']} Revoked")
    print(f"Manual Action Items:{data['requiresManualActionCount']}")
    
    print(f"\n{Colors.BOLD}Provider Breakdown:{Colors.ENDC}")
    for provider, res in data.get('providerResults', {}).items():
        p_color = Colors.GREEN if res['status'] == 'CONTAINED' else Colors.WARNING
        print(f"  • {res['providerName']:<24}: {p_color}{res['status']:<10}{Colors.ENDC} - {res['details']}")
...
def cmd_rollback(args):
    ...
    print(f"{Colors.GREEN}✓ Identity {res['username']} restored to ACTIVE state!{Colors.ENDC}")
